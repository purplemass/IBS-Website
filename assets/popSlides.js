		...
		this.each(function() {
			...
			function init() {
				...
				$(px).css({
					...
					'width':'15px',
					'height':'15px',
					'background':'url("'+options.path+'popSlides_sprites.png") -50px 0 no-repeat'
				});
				$(pp).css({
					'cursor':'pointer',
					'position':'absolute',
					...
					'width':'15px',
					'height':'15px',
					'background':'url("'+options.path+'popSlides_sprites.png") -50px -15px no-repeat'
				});
				if (play) {
					$(pp).css('background','url("'+options.path+'popSlides_sprites.png") -65px -15px no-repeat');
				}
				$(pl).css({
					'background':'url("'+options.path+'popSlides_sprites.png") 0 0 no-repeat',
					'height':'30px',
					'width':'25px',
					'position':'absolute',
					'top':'50%',
				...
				$(pr).css({
					'background':'url("'+options.path+'popSlides_sprites.png") -25px 0 no-repeat',
					'height':'30px',
					'width':'25px',
					'position':'absolute',
					'top':'50%',
			...
			function togglePlay() {
				if (play) {
					$(bar).stop();
					$(bar).fadeOut(300);
					$(pp).css('background','url("'+options.path+'popSlides_sprites.png") -50px -15px no-repeat');
					play = false;
				} else {
					$(pp).css('background','url("'+options.path+'popSlides_sprites.png") -65px -15px no-repeat');
					play = true;
